...
import professionalAxios from "../../../../services/professionalAxios";
import { getCompany } from "../professionalCompanyMaster.slice";
// ⭐ YELLOW STAR: ADDED — REUSE THE EXISTING COMPANY MASTER THUNK
...
export const getEWayBillByNumber = createAsyncThunk(
    "driverSettlement/getEWayBillByNumber",
    async (eWayBillNumber: string, { rejectWithValue, dispatch, getState }) => {
        try {
            // ⭐ YELLOW STAR: ADDED — COMPANY MASTER GST NUMBER
            const ewbCredentials = await getCompanyMasterEWayBillCredentials(dispatch, getState);
            const response = await professionalAxios.get(
...
/* ===================================================
    COMPANY MASTER E-WAY BILL CREDENTIALS
=================================================== */

// ⭐ YELLOW STAR: UPDATED — USE COMPANY MASTER FOR ALL THREE VALUES, KEEP CURRENT DEFAULTS
const getCompanyMasterEWayBillCredentials = async (
    dispatch: any,
    getState: any,
    fallbackCredentials: {
        gstin: string;
    ...
    try {
        ...
        return {
            ...
            ewbpwd:
                String(company?.ewbpwd || "").trim() ||
                fallbackCredentials.ewbpwd,
        };
    } catch {
        // ⭐ YELLOW STAR: COMPANY MASTER FAILURE PRESERVES THE EXISTING CREDENTIALS
        return {
            gstin: fallbackCredentials.gstin,
            username: fallbackCredentials.username,
            ewbpwd: fallbackCredentials.ewbpwd,
        };
    }
};
