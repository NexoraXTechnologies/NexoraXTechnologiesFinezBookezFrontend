...
// /* ===================================================
//    GET Consolidated vehicles
// =================================================== */
// export const getConsolidatedVehicles = createAsyncThunk(
//     "whereIsMyDriver/getConsolidatedVehicles",
//     async ({ limit = 100, offset = 0 }: TripTrackingParams = {},
//         { rejectWithValue }
//     ) => {
//         try {
//             const response = await professionalAxios.get("/eTaxSolnMongoApiBackend/users/bookEZ/tripTracking/consolidateVehical", {
//                 params: {
//                     limit, offset
//                 }
//             });
...
//         } catch (error: any) {
//             return rejectWithValue({
//                 message: error?.response?.data?.message || error?.message || "Failed to load consolidated Vehicle"
//             })
//         }
//     }
// )
...
//             .addCase(getConsolidatedVehicles.pending, (state) => {
//                 state.listingLoader = true;
//                 state.error = null;
//             })

//             .addCase(getConsolidatedVehicles.fulfilled, (state, action) => {
//                 state.listingLoader = false;
//                 state.consolidatedVehicle = action.payload?.records || [];
//                 state.error = null;
//             })
//             .addCase(getConsolidatedVehicles.rejected, (state, action) => {
//                 state.listingLoader = false;
//                 state.consolidatedVehicle = [];
//                 state.error = (action.payload as { message?: string })?.message || "Failed to load consolidated vehicle";
//             })
//     },
// });
...
export const getWhereIsMyDriverList = createAsyncThunk(
    ...
    async (
        ...
        try {
            ...
            const tripList = (Array.isArray(list) ? list : []).filter((item: any) => {
                return (
                    Number.isFinite(Number(item?.currentLocation?.lat)) &&
                    Number.isFinite(Number(item?.currentLocation?.lng))
                );
            ...
            return {
                drivers: driversWithAddress,
                totalActiveRecords: tripList.length,
            };
        } catch (error: any) {
            return rejectWithValue({
                message:
...
export const getConsolidatedVehicles = createAsyncThunk(
    ...
    async (
        ...
        try {
            const response = await professionalAxios.get(
                "/eTaxSolnMongoApiBackend/users/bookEZ/tripTracking/consolidateVehical",
                {
                    params: {
                        limit,
                        offset
            ...
            return rejectWithValue({
                message:
                    error?.response?.data?.message ||
                    error?.message ||
                    "Failed to load consolidated Vehicle"
            });
        }
    }
);
