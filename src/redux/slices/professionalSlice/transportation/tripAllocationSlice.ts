...
/* ===================================================
   SYNC ALLOCATION STATUS ON TRIP COMPLETE
=================================================== */

export const syncAllocationStatusOnComplete = createAsyncThunk(
	"tripAllocation/syncAllocationStatusOnComplete",
	async (voucherNumber: string, { dispatch, rejectWithValue }) => {
		try {
			if (!voucherNumber) {
				return rejectWithValue({ message: "Trip allocation voucher number not found" });
			}
			...
			return rejectWithValue({
				message:
					error?.response?.data?.message ||
					error?.message ||
					"Failed to sync trip allocation status",
			});
		}
	}
);
...
export const isDriverSelectableStatus = (status: any) => {
	...
	return DRIVER_VEHICLE_STATUS_OPTIONS.some(
		(option: any) => option.value === normalized
	);
};

export const readVehicleStatusFromRecord = (record: any) =>
...
export const getVehicleVoucherFromTripExpense = (form: any) =>
	String(
		form?.vehicle?.selectedVehicleId ||
		form?.vehicle?.vehicleVoucherNumber ||
		form?.vehicle?.voucherNumber ||
		form?.vehicleSelection?.selectedVehicleId ||
		form?.vehicleSelection?.voucherNumber ||
		form?.selectedVehicleId ||
		form?.vehicleVoucherNumber ||
		""
	).trim();

const loadVehicleMasterRecords = async () => {
	const moduleResponse = await professionalAxios.get(
		"/eTaxSolnMongoApiBackend/users/customMaster/module/getAll",
		{
			params: {
...
export const getVehicleMasterByVoucher = async (vehicleVoucher: string) => {
	const voucher = String(vehicleVoucher || "").trim();

	if (!voucher) return null;

	const { records } = await loadVehicleMasterRecords();
	...
	return (
		records.find((item: any) => {
			...
			return String(itemVoucher).trim() === voucher;
		}) || null
	);
};

export const updateVehicleMasterStatus = async ({
	vehicleVoucher,
	nextStatus,
}: {
	vehicleVoucher: string;
	nextStatus: string;
}) => {
	const voucher = String(vehicleVoucher || "").trim();

	if (!voucher || !nextStatus) {
		throw new Error("Vehicle voucher and status are required");
	}

	const currentRecord = await getVehicleMasterByVoucher(voucher);

	if (!currentRecord) {
		throw new Error("Vehicle master record not found");
	}

	const safeSystemStatus =
		currentRecord?.status ||
		currentRecord?.data?.status ||
		currentRecord?.dynamicFields?.status ||
		"active";
	...
	const payload = {
		...
		vehicle_status: nextStatus,
		availability: nextStatus,

		// IMPORTANT: do not put Loading/In-Transit here
		...
		data: {
			...
			vehicle_status: nextStatus,
			availability: nextStatus,
			status: currentRecord?.data?.status || safeSystemStatus,
		},

		dynamicFields: {
			...(currentRecord?.dynamicFields || {}),
			current_status: nextStatus,
			currentStatus: nextStatus,
			availabilityStatus: nextStatus,
			availability_status: nextStatus,
			...
			vehicle_status: nextStatus,
			availability: nextStatus,
			status: currentRecord?.dynamicFields?.status || safeSystemStatus,
		},
	};

	const response = await professionalAxios.put(
		`/eTaxSolnMongoApiBackend/users/customMaster/data/update/${voucher}`,
		payload
	);

	if (response?.data?.success === false) {
		throw new Error(response?.data?.message || "Vehicle status update failed");
	}
...
const tripAllocationSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(syncAllocationStatusOnComplete.pending, (state) => {
				state.updateLoader = true;
				state.error = null;
			})
			.addCase(syncAllocationStatusOnComplete.fulfilled, (state, action) => {
				state.updateLoader = false;
			...
			.addCase(syncAllocationStatusOnComplete.rejected, (state, action) => {
				state.updateLoader = false;
				state.error =
					(action.payload as { message?: string })?.message ||
					"Failed to sync allocation status";
			})
