...
const OTHER_INCOME_OPTIONS: { label: string; value: string }[] = [
...
const IncomeOtherSource = ({ value, assessmentYear, onClose, onSave }: { value?: any; assessmentYear?: string; onClose: () => void; onSave: (data: any) => void }) => {
  /* -------- Nature of Income -------- */
  ...
  const totalOtherIncome = useMemo(() => otherRows.reduce((sum: any, r: any) => sum + toNum(r.amount), 0), [otherRows]);
  ...
  const total89AQuarter = useMemo(() => Object.values(quarters89A).reduce((a: any, b: any) => a + toNum(b), 0), [quarters89A]);

  const totalDividend = useMemo(() => Object.values(dividend).reduce((a: any, b: any) => a + toNum(b), 0), [dividend]);
  ...
  const payload = {
    ...
    otherRows: otherRows.filter((r: any) => r.type || r.amount).map((r: any) => ({ type: r.type, amount: toNum(r.amount) })),
      ...
      <div className="bg-white w-[82%] max-w-[1100px] rounded-xl shadow-xl max-h-[88vh] overflow-y-auto p-5">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <RupeeInput value={nature.saving} onChange={(v: any) => setNature({ ...nature, saving: v })} />
          </div>
          <div>
            <label className="text-xs">Interest from Deposits</label>
            <RupeeInput value={nature.deposit} onChange={(v: any) => setNature({ ...nature, deposit: v })} />
          </div>
          <div>
            <label className="text-xs">Interest on Income Tax Refund</label>
            <RupeeInput value={nature.refund} onChange={(v: any) => setNature({ ...nature, refund: v })} />
          </div>
        </div>
            ...
            <button
              onClick={() => {
                if (otherRows.length === 1) {
                  ...
                  setOtherRows(otherRows.filter((_: any, idx: any) => idx !== i));
                }
              }}
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <RupeeInput value={nonNotified89A} onChange={(v: any) => setNonNotified89A(v)} />
          </div>
        </div>
        {/* Notified Country */}
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <RupeeInput value={notified.usa} onChange={(v: any) => setNotified({ ...notified, usa: v })} />
              </div>

              <div>
                <label className="text-xs">Notified u/s 89A(ii) – UK</label>
                <RupeeInput value={notified.uk} onChange={(v: any) => setNotified({ ...notified, uk: v })} />
              </div>

              <div>
                <label className="text-xs">Notified u/s 89A(iii) – Canada</label>
                <RupeeInput value={notified.canada} onChange={(v: any) => setNotified({ ...notified, canada: v })} />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              {QUARTERS.map(({ key, label }) => (
                <div key={key}>
                  ...
                  <RupeeInput value={quarters89A[key]} onChange={(v: any) => setQuarters89A({ ...quarters89A, [key]: v })} />
                </div>
              ))}
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {QUARTERS.map(({ key, label }) => (
            <div key={key}>
              <label className="text-xs">{label}</label>
              <RupeeInput value={dividend[key]} onChange={(v: any) => setDividend({ ...dividend, [key]: v })} />
            </div>
          ))}
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <RupeeInput value={deductions.deduction57} onChange={(v: any) => setDeductions({ ...deductions, deduction57: v })} />
          </div>
          <div>
