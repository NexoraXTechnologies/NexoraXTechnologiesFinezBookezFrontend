...
const CreditNote = () => {
    ...
    const unitOptions = useMemo(() => {
        return (
            unitRows?.map((item: any) => ({
                label: item.unitName || item.unit || item.unitId,
                value: item.unitId || item.unit || item.unitName,
                raw: item,
            })) || []
        );
    }, [unitRows]);
        ...
        setForm((prev: any) => {
            ...
            if (key === "productCode") {
                const product = productOptions.find(
                    (item: any) => String(item.productCode) === String(value)
                );
        ...
        <>
            <div className="flex h-full w-full flex-col border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <DataTable
                    columns={mainColumns}
                    data={creditNoteList}
                    loading={listingLoader || dropdownLoading}
