...
const OpeningStock = () => {
    ...
    const dynamicFooterArray = useMemo(() => {
        return (templateFields?.footer ||
            ...
            .map((field: any) => {
                const rawValue = footerTotals?.[field.key] ?? 0;
                return {
                    ...field,
    ...
    return (<>
        ...
        {!fieldsLoading && (<DynamicAddForm show={showModal} setShow={setShowModal} edit={edit} title="Opening Stock" subtitle="Fill in the opening stock details below" loading={addLoader ||
            updateLoader} onClose={() => {
                setShowModal(false);
                resetMainForm();
            }} onSubmit={handleSubmit} form={form} errors={errors} handleAddRow={handleAddRow} handleDeleteRow={handleDeleteRow} handleRowChange={handleRowChange} inputData={dynamicInputData} bodyKey="openingStockBody" handleChange={handleChange} footerTotals={footerTotals} />)}
    </>);
};
