...
const ReceiptFromProduction = () => {
    ...
    const dynamicFooterArray = useMemo(() => {
        return (templateFields?.footer || []).map((field: any) => {
            ...
            if (["totalProducedQuantity", "totalQuantity", "totalQty"].includes(key)) rawValue = footerTotals.totalProducedQuantity;
            if (["totalFinishedCost", "totalAmount", "grossAmount"].includes(key)) rawValue = footerTotals.totalFinishedCost;
    ...
    const fetchReceiptFromProductionList = async () => {
        try {
            await dispatch(
                getReceiptFromProductionList({ offset: localOffset, limit: localLimit, search: debouncedSearch }) as any
            ).unwrap();
        } catch (error: any) {
    ...
    useEffect(() => {
        const prepareFields = async () => {
            ...
            try {
                ...
                if (!editingVoucherNumber) setForm(buildBlankForm(updatedData));
            } catch (error) {
    ...
    const buildFormFromRecord = (record: any) => {
        ...
        [...(templateFields?.header || []), ...(templateFields?.headerChild || [])].forEach((field: any) => {
            ...
            if (field.key === "issueVoucherNumber") value = record?.reference?.issueVoucherNumber ?? record?.issueVoucherNumber ?? value;
            if (field.key === "issueId") value = record?.reference?.issueId ?? record?.issueId ?? value;
        ...
        next.reference = {
            issueVoucherNumber: record?.reference?.issueVoucherNumber || record?.issueVoucherNumber || "",
            issueId: record?.reference?.issueId || record?.issueId || record?.reference?.issueVoucherNumber || "",
        };
        ...
        next.finishedGoods = Array.isArray(record?.finishedGoods) && record.finishedGoods.length
            ? record.finishedGoods.map((item: any) => ({ ...buildBlankRow(templateFields?.body || []), ...item, id: item?.id || Date.now() + Math.random() }))
    ...
    const openEditModal = async (record: any) => {
        ...
        try {
            ...
            const detail = responseData?.record || responseData?.receiptFromProduction || responseData || record;
            ...
            toast.error(error?.response?.data?.message || error?.message || "Failed to load Receipt From Production");
        } finally {
            setFieldsLoading(false);
        }
    };
            ...
            if (key === "issueVoucherNumber") {
                updated = {
                    ...
                    reference: {
                        ...
                        issueId: previous?.reference?.issueId || previous?.issueId || value,
                    },
                };
            }
    ...
    const handleAddRow = () => {
        setForm((previous: any) => ({
            ...previous,
            finishedGoods: [...(previous?.finishedGoods || []), buildBlankRow(templateFields?.body || [])],
        }));
    };

    const handleDeleteRow = (index: number) => {
        setForm((previous: any) => {
            const rows = (previous?.finishedGoods || []).filter((_: any, rowIndex: number) => rowIndex !== index);
            return { ...previous, finishedGoods: rows.length ? rows : [buildBlankRow(templateFields?.body || [])] };
        });
    };
    ...
    const handleRowChange = (index: number, key: string, value: any) => {
        ...
        setForm((previous: any) => {
            ...
            if (PRODUCT_FIELD_KEYS.has(key)) {
                ...
                row.rate = row?.rate || raw?.sellingPrice || raw?.productSellingPrice || raw?.salesRate || raw?.saleRate || raw?.rate || "";
            }

            if (key === "quantity" || key === "rate" || PRODUCT_FIELD_KEYS.has(key)) row = calculateFinishedGoodRow(row);

            rows[index] = row;
            return { ...previous, finishedGoods: rows };
        });

        setErrors((previous: any) => ({ ...previous, finishedGoods: "", [`row_${index}_${key}`]: "" }));
    };
    ...
    const validateForm = () => {
        ...
        if (!rows.length) nextErrors.finishedGoods = "Please add at least one finished good";
        ...
        if (nextErrors.finishedGoods) toast.error(nextErrors.finishedGoods);
    ...
    const buildPayload = () => {
        ...
        [...(templateFields?.header || []), ...(templateFields?.headerChild || [])].forEach((field: any) => {
            ...
            if (["issueVoucherNumber", "issueId"].includes(field.key)) return;
        ...
        (templateFields?.footer || []).forEach((field: any) => {
            ...
            if (["totalProducedQuantity", "totalQuantity", "totalQty"].includes(field.key)) {
                payload[field.key] = String(footerTotals.totalProducedQuantity);
            } else if (["totalFinishedCost", "totalAmount", "grossAmount"].includes(field.key)) {
                payload[field.key] = String(footerTotals.totalFinishedCost.toFixed(2));
            } else if (form?.[field.key] !== undefined) {
                payload[field.key] = form[field.key];
            }
        });

        const issueVoucherNumber = form?.reference?.issueVoucherNumber || form?.issueVoucherNumber || "";
        const issueId = form?.reference?.issueId || form?.issueId || issueVoucherNumber || "";

        payload.voucherDate = form?.voucherDate || todayYMD();
        payload.status = form?.status || "open";
        payload.transactionType = form?.transactionType || "RECEIPT_FROM_PRODUCTION";
        payload.reference = { issueVoucherNumber, issueId };
        payload.finishedGoods = cleanFinishedGoods();
        payload.totalProducedQuantity = String(footerTotals.totalProducedQuantity);
        payload.totalFinishedCost = String(footerTotals.totalFinishedCost.toFixed(2));

        if (form?.headerRemarks !== undefined) payload.headerRemarks = form.headerRemarks;
        if (form?.warehouseCode !== undefined) payload.warehouseCode = form.warehouseCode;
        if (form?.locationCode !== undefined) payload.locationCode = form.locationCode;
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editingVoucherNumber) {
                await dispatch(
                    updateReceiptFromProduction({ payload, receiptFromProductionVoucherNumber: editingVoucherNumber }) as any
                ).unwrap();
                toast.success("Receipt From Production updated successfully");
            } else {
                await dispatch(addReceiptFromProduction({ payload }) as any).unwrap();
                toast.success("Receipt From Production created successfully");
            }
            ...
            toast.error(error?.message || "Receipt From Production operation failed");
        }
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await dispatch(
                deleteReceiptFromProduction({ receiptFromProductionVoucherNumber: confirmTooltip.voucherNumber }) as any
            ).unwrap();
            toast.success("Receipt From Production deleted successfully");
            await fetchReceiptFromProductionList();
        } catch (error: any) {
            toast.error(error?.message || "Failed to delete Receipt From Production");
        } finally {
    ...
    const columns = [
        ...
        { key: "voucherDate", title: "Date", render: (row: any) => row?.voucherDate ? formatDateForList(row.voucherDate) : "-" },
        {
            ...
            render: (row: any) => row?.reference?.issueVoucherNumber || row?.issueVoucherNumber || "-",
        },
        { key: "finishedGoods", title: "Finished Goods", render: (row: any) => row?.finishedGoods?.length || 0 },
        { key: "totalProducedQuantity", title: "Produced Qty", render: (row: any) => row?.totalProducedQuantity ?? "0" },
        {
            key: "totalFinishedCost",
            title: "Finished Cost",
            render: (row: any) => <span className="font-medium text-card-foreground">{money(row?.totalFinishedCost || 0)}</span>,
        },
        {
            key: "status",
            title: "Status",
            render: (row: any) => (
                <span className={`rounded-md border px-2 py-1 text-xs font-medium capitalize ${row?.status === "open" ? "border-success/20 bg-success/10 text-success" : "border-danger/20 bg-danger/10 text-danger"}`}>
                    {row?.status || "-"}
                </span>
            ),
        },
        ...
        <div className="flex h-full w-full flex-col rounded-md border border-border bg-card p-4 text-card-foreground shadow-sm">
            <div className="mb-3 flex items-center">
                <div className="flex items-start gap-3">
                    <Badge count={pagination?.totalDocs ?? 0} text="Total Receipt From Production:" varient="primary" />
                </div>
                ...
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <Permission module="bookez" permissionKey="productions.receiptFromProduction" action="create">
                        <DataCreateButton callBackFn={openAddModal} text="Add Receipt From Production" />
                    </Permission>
                </div>
            </div>
            ...
            <DataTable
                ...
                actions={(record: any) => (
                    <div className="flex items-center gap-2">
                        <Permission module="bookez" permissionKey="productions.receiptFromProduction" action="update">
                            <button
                        ...
                        <Permission module="bookez" permissionKey="productions.receiptFromProduction" action="delete">
                            <button
