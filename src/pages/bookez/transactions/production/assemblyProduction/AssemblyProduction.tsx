...
/* ===================================================
    ...
    Used for:
    - Finished good default form
    - Raw material default form
=================================================== */
const emptyProduct = {
    productCode: "",
    productName: "",
...
/* ===================================================
    ...
    This creates a fresh assembly production form.
=================================================== */
const getDefaultForm = () => ({
    voucherNumber: "ASP",
    voucherDate: new Date().toISOString().split("T")[0],
...
const AssemblyProduction = () => {
    ...
    /* ===================================================
        ...
        All assembly production list/loading states are kept here.
    =================================================== */
    const {
        assemblyProductions,
        pagination,
    ...
    /* ===================================================
        ...
        showModal              => Main add/update assembly modal
        showRawMaterialModal   => Raw material add/update modal
        showNoRawMaterialAlert => Warning if no raw material product found
    =================================================== */
    const [showModal, setShowModal] = useState(false);
    const [showRawMaterialModal, setShowRawMaterialModal] = useState(false);
    const [showNoRawMaterialAlert, setShowNoRawMaterialAlert] = useState(false);
    ...
    */
    ...
    /*
      Used to make sure user clicks Apply before saving.
    */
    const [isFinishedGoodApplied, setIsFinishedGoodApplied] = useState(false);

    /*
      Raw material modal form state.
    */
    const [rawMaterialForm, setRawMaterialForm] = useState<any>({
        ...emptyProduct,
    });
    ...
    /* ===================================================
        ...
        errors    => Main assembly form errors
        rawErrors => Raw material modal errors
    =================================================== */
    const [errors, setErrors] = useState<any>({});
    const [rawErrors, setRawErrors] = useState<any>({});
    ...
    /* ===================================================
        ...
        finishedGoodOptions => Product dropdown for finished goods
        rawMaterialOptions  => Product dropdown for raw materials
    =================================================== */
    const [finishedGoodOptions, setFinishedGoodOptions] = useState<any[]>([]);
    const [rawMaterialOptions, setRawMaterialOptions] = useState<any[]>([]);
    ...
    /*
      Get product rate from different possible product price keys.
    */
    const getProductRate = (product: any, fallback = "") => {
        return String(
            product?.sellingPrice ||
    ...
    /*
      Convert product API response into SelectInput options.
      This keeps same flexible response handling as original code.
    */
    const makeProductOptions = (res: any) => {
        const records = Array.isArray(res?.items)
            ? res.items
    ...
    /*
      Calculate amount using quantity and rate.
    */
    const calculateAmount = (quantity: any, rate: any) => {
        const qty = Number(quantity || 0);
        const price = Number(rate || 0);
    ...
    /* ===================================================
        ...
        Auto calculated from form.rawMaterials.
    =================================================== */
    const totalRawMaterialCost = useMemo(() => {
        return form.rawMaterials.reduce((sum: number, item: any) => {
            return sum + Number(item.amount || 0);
    ...
    /* ===================================================
        ...
        totalRawMaterialCost + productionCost
    =================================================== */
    const totalFinishedCost = useMemo(() => {
        return totalRawMaterialCost + Number(form.productionCost || 0);
    }, [totalRawMaterialCost, form.productionCost]);
    ...
    /* ===================================================
        ...
        Wait 400ms after typing before API call.
    =================================================== */
    useEffect(() => {
        const timer = setTimeout(() => {
            setDebouncedSearch(search.trim());
    ...
    /* ===================================================
        ...
        Reset all form states before opening add modal.
    =================================================== */
    const openAddModal = () => {
        setEditingRecord(null);
        setErrors({});
    ...
    /* ===================================================
        ...
        Hydrates form from selected record.
    =================================================== */
    const openEditModal = (record: any) => {
        setEditingRecord(record);
        setErrors({});
        ...
        setForm({
            ...
            /*
              ...
              productionCost comes from record.productionCost,
              fallback to finished good rate.
            */
            productionCost:
                record?.productionCost ||
                record?.finishedGood?.rate ||
    ...
    /* ===================================================
        ...
        If user changes anything after Apply,
        then user must click Apply again.
    =================================================== */
    const handleFinishedGoodChange = (key: string, value: any) => {
        setFinishedGoodForm((prev: any) => {
            const updated = {
    ...
    /* ===================================================
        OLD APPLY FINISHED GOOD CODE

        Kept as comment from original code.
        Do not remove because user asked not to remove anything.
    =================================================== */
    // const handleApplyFinishedGood = () => {
    //     if (!validateFinishedGood()) return;
    ...
    /* ===================================================
        ...
        - Apply finished good into main form
        - Production cost auto-fetched from finished good rate
    =================================================== */
    const handleApplyFinishedGood = () => {
        if (!validateFinishedGood()) return;
    ...
    /* ===================================================
        ...
        Opens raw material modal in edit mode.
    =================================================== */
    const handleEditRawMaterial = (item: any, index: number) => {
        setEditingRawMaterialIndex(index);
    ...
    /* ===================================================
        ...
        If editingRawMaterialIndex is not null:
    ...
    /* ===================================================
        ...
        Before saving assembly production:
        - Date required
        - Status required
        - Finished good must be applied
        - Finished good product/qty/rate required
    ...
    /* ===================================================
        OLD SAVE / UPDATE ASSEMBLY PRODUCTION CODE
    ...
    /* ===================================================
        ...
        Current functionality:
        - Validate form
        - Remove voucherNumber from payload
        - Calculate final amounts
        - Dispatch create/update API
    ...
    /* ===================================================
        ...
        Used inside AssemblyProductionFormModal.
    =================================================== */
    const rawMaterialTableFields = [
        {
            title: "Name",
    ...
    return (
        <div className="w-full bg-white border border-gray-200 rounded-md shadow-sm p-4 flex flex-col h-[100%]">
            ...
            <AssemblyProductionFormModal
                ...
                handleFinishedGoodProductChange={handleFinishedGoodProductChange}
