...
const SalesReturn = () => {
    ...
    const getInventoryBalanceFilters = (row: any) => {
        ...
        const visibleBodyFields =
            (templateFields?.body || []).filter(
                (field: any) =>
                    !isTrueValue(field?.isHidden)
            );

        const visibleHeaderFields =
            (templateFields?.header || []).filter(
                (field: any) =>
                    !isTrueValue(field?.isHidden)
            );
        ...
        const inventoryApiKeys = new Set(
            [
                ...
                .map(
                    (field: any) =>
                        getInventoryBalanceApiKey(field)
                )
                .filter(Boolean)
        );
        ...
        inventoryApiKeys.forEach((apiKey: any) => {
            const bodyField =
                visibleBodyFields.find(
                    (field: any) =>
                        getInventoryBalanceApiKey(field) === apiKey
                );

            if (bodyField) {
                const selectedFilters =
                    row?._inventoryBalanceSelections &&
                        typeof row._inventoryBalanceSelections ===
                        "object"
                        ? row._inventoryBalanceSelections
                        : {};
    ...
    const getInventoryFieldValue = (
        ...
        const masterName =
            getCustomMasterName(schemaField);

        const selectedMaster =
            source?.customMasters?.[masterName] ||
            source?.customMasters?.[
            schemaField.key
            ];
    ...
    const getInventoryTransactionValue = (
        row: any,
        apiKey: string
    ) => {
        const visibleBodyFields =
    ...
    const getInventoryBalanceVoucherId = (
        row: any
    ) =>
        String(
    ...
    const getInventoryBalanceRecords = (
        response: any
    ) => {
        const data =
            response?.data?.data ??
    ...
    const resolveSavedSalesReturnVoucherNumber = (
        ...
        const responseVoucherNumber =
            findVoucherNumber(
                response
            );

        if (responseVoucherNumber)
            return responseVoucherNumber;
    ...
    // PARTIAL SALES RETURN
    const getSalesReturnAnalysisData = (
        summary: any
    ) => {
        return (
            summary?.data?.data ??
    ...
    // PARTIAL SALES RETURN
    const getSalesReturnProductWiseDetail = (
        summary: any
    ) => {
        const data =
            getSalesReturnAnalysisData(
                summary
            );

        if (
            Array.isArray(
                data?.productWiseDetail
            )
        ) {
            return data.productWiseDetail;
        }
        ...
        if (
            Array.isArray(
                data?.pending?.products
            )
        ) {
            return data.pending.products;
        }

        return [];
    };

    const getPendingReturnQtyFromAnalysis = (
        summary: any
    ) => {
        const data =
            getSalesReturnAnalysisData(
        ...
        const productWiseDetail =
            getSalesReturnProductWiseDetail(
                data
            );

        if (
            productWiseDetail.length >
            0
        ) {
            return productWiseDetail.reduce(
                (
                    sum: number,
                    item: any
                ) =>
    ...
    const syncSalesInvoiceStatusAfterReturnAnalysis =
        async (
            ...
            try {
                const summary =
                    await dispatch(
                        getSalesReturnAnalysisByInvoiceVoucher({
                            voucherNumber:
                                sInvVoucherNumber,
                        }) as any
                    ).unwrap();
                ...
                const totalPendingQuantity =
                    getPendingReturnQtyFromAnalysis(
                        summary
                    );
    ...
    const columns = [
        ...
        },
        {
            ...
            render:
                (row: any) => (
                    <span
                        className={`rounded-md border px-2 py-1 text-xs font-medium capitalize ${(row?.sInvReturnStatus ||
                            row?.sInvStatus) ===
                            "open"
    ...
    const handleRowChange = (
        ...
        setForm(
            (prev: any) => {
                ...
                if (
                    isInventoryBalanceField(
                        currentField
                    )
                ) {
                    const apiKey =
                        getInventoryBalanceApiKey(
                            currentField
                        );
                    ...
                    const currentSelections =
                        updatedRow?._inventoryBalanceSelections &&
                            typeof updatedRow._inventoryBalanceSelections ===
                            "object"
                            ? {
                                ...updatedRow._inventoryBalanceSelections,
                            }
                ...
                if (
                    ...
                    const customMasters = {
                        ...(currentRow
                            ?.customMasters &&
                            typeof currentRow.customMasters ===
                            "object"
        ...
        (
            ...
            (
                ...
                if (
                    ...
                    row?.salesReturnPendingQuantity !==
                    null &&
                    row?.salesReturnPendingQuantity !==
                    undefined &&
                    num(
                    ...
                    num(
                        row
                            ?.salesReturnPendingQuantity
                    )
                ) {
                    err[
                        `row_${index}_quantity`
                    ] =
                        `Quantity cannot exceed pending Sales Return quantity ${num(
                            row
                                ?.salesReturnPendingQuantity
                        )}`;
                }
    ...
    const handleDeleteConfirm =
        async () => {
            try {
                ...
                const linkedInvoiceVoucherNumber =
                    deletedRecord?.sInvVoucherNumber ||
                    "";
    ...
    const handlePurchaseOrderConfirm =
        async () => {
            ...
            try {
                ...
                console.log(
                    "Failed to load Sales Return pending quantity",
                    error
                );
                    ...
                    ? poBody
                        .map(
                            (
                                ...
                                const baseRow = {
                                    ...
                                    // PARTIAL SALES RETURN - FRONTEND ONLY
                                    salesReturnPendingQuantity:
                                        rowPendingQuantity,

                                    salesReturnInvoiceQuantity:
                                        invoiceQuantity,
                        ...
                        .filter(
                            (
                                ...
                                !item?.productCode ||
                                num(
                                    item?.salesReturnPendingQuantity
                                ) > 0
                        )
                    : [
                        {
            ...
            if (
                ...
                toast.info(
                    "This Sales Invoice is already fully returned"
                );
    ...
    const handleDeleteSalesInvoiceClick = (
        e: any,
        record: any
    ) => {
        if (
    ...
    const headerInventoryBalanceSignature =
        useMemo(() => {
            return (
                templateFields?.header ||
                []
    ...
    useEffect(() => {
        ...
        const fetchAvailableQuantities =
            async () => {
                const {
                    fromDate,
                    toDate,
            ...
            {!fieldsLoading && (
                <DynamicAddForm
                    {...{
                        ...
                        subtitle:
                            "Fill in the sales invoice details below",
