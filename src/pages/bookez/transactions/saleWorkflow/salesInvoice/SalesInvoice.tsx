...
const getDynamicFieldType = (field: any) => String(field?.type ||
...
const getCustomMasterName = (field: any) => String(field?.customMasterName ||
...
export const loadFieldOptions = async (fields: any[]) => {
    const updatedFields = await Promise.all((fields || []).map(async (field) => {
        ...
        try {
            const res = await professionalAxios.get(`/eTaxSolnMongoApiBackend${apiUrl}`, {
...
const renderSalesInvoiceCellExtra = (column: any, row: any, enableServiceProductInventory: boolean) => {
    ...
    if (productType === "serviceproduct" &&
        !enableServiceProductInventory) {
        return null;
    }
    return (<InputBorderLabel label="Avl Qty" value={row?.availableQuantity} loading={row?.availableQuantity === null ||
        row?.availableQuantity === undefined} successWhenPositive />);
};
const SalesInVoice = () => {
        ...
        return {
            ...
            header: (templateFields?.header || []).map((field: any) => {
                ...
                return {
                    ...
                    createOptionLabel: (searchValue: string) => searchValue
    ...
    const getInventoryBalanceFieldValue = (row: any, field: any) => {
        ...
        const selectedMaster = row?.customMasters?.[customMasterName] ||
            row?.customMasters?.[field?.key];
        const rawValue = selectedMaster?.code ??
            row?.[field?.key] ??
    ...
    const getInventoryFieldValue = (source: any, fields: any[], apiKey: string) => {
        ...
        const selectedMaster = source?.customMasters?.[customMasterName] ||
            source?.customMasters?.[schemaField.key];
        const value = selectedMaster?.code ??
    ...
    const getInventoryTransactionValue = (row: any, apiKey: string) => {
        ...
        if (bodyField) {
            return getInventoryFieldValue(row, visibleBodyFields, apiKey);
        }
        const headerField = visibleHeaderFields.find((field: any) => getInventoryTransactionApiKey(field) === apiKey);
        if (headerField) {
            return getInventoryFieldValue(form, visibleHeaderFields, apiKey);
        }
        return "";
    };
    const getInventoryBalanceVoucherId = (row: any) => String(row?._inventoryBalanceVoucherId ||
    ...
    const attachInventoryBalanceVoucherIds = (rows: any[], inventoryRecords: any[]) => {
        ...
        return (rows || []).map((row: any) => {
            ...
            const exactRecord = availableRecords.find((record: any) => String(record?.warehouseCode || "") === warehouseCode &&
    ...
    const buildInventoryBalancePayload = (row: any, voucherNumber: string) => {
        const invoiceStatus = String(form?.sInvStatus ||
            form?.sInvDocStatus ||
            "open")
            .trim()
            .toLowerCase();
        const inventoryStatus = ["close", "closed", "cancelled"].includes(invoiceStatus)
            ? "inactive"
            : "active";
        return {
            voucherNumber,
            voucherNumberSnapshot: form?.sInvVoucherNumber &&
    ...
    const syncInventoryBalance = async (voucherNumber: string, isEdit: boolean) => {
        ...
        for (const row of rows) {
            ...
            if (inventoryBalanceVoucherId) {
                ...
                await dispatch(updateInventoryBalance({
    ...
    const calculateRow = (row: any, changedKey = "") => {
        ...
        const nonTaxGross = marginProduct
            ...
            : 0;
        const changedDiscountKey = String(changedKey || "")
            .trim()
            .toLowerCase();
        const isDiscountPercentChanged = changedDiscountKey === "discount" ||
        ...
        const discountPercentSource = changedDiscountKey === "discount"
            ...
            : changedDiscountKey === "discountpercentage"
                ...
                : row.discountPercentage !== undefined &&
                    ...
                    ? row.discountPercentage
                    : row.discount;
        const hasDiscountPercent = discountPercentSource !== undefined &&
            discountPercentSource !== null &&
            discountPercentSource !== "";
        const hasDiscountAmount = row.discountAmount !== undefined &&
            row.discountAmount !== null &&
            row.discountAmount !== "";
        let discountPercent = hasDiscountPercent
            ? safePercent(discountPercentSource)
            : 0;
        const cgstPercent = safePercent(row.cgstPercentage !== undefined &&
            row.cgstPercentage !== null &&
            row.cgstPercentage !== ""
            ? row.cgstPercentage
            : row.cgst);
        const sgstPercent = safePercent(row.sgstPercentage !== undefined &&
            row.sgstPercentage !== null &&
            row.sgstPercentage !== ""
            ? row.sgstPercentage
            : row.sgst);
        const igstPercent = safePercent(row.igstPercentage !== undefined &&
        ...
        const discountPercentageValue = hasDiscountPercent ||
            hasDiscountAmount
            ? discountPercent
            : "";
        return {
    ...
    const columns = [
        ...
        { key: "sInvBody", title: "Items", render: (row: any) => row?.sInvBody?.length || 0 },
        {
            key: "sInvFooter",
            title: "Net Amount",
            render: (row: any) => (<span className="font-semibold text-primary">
        ...
        },
        {
            key: "sInvDocStatus",
            title: "Doc Status",
            render: (row: any) => (<span className={`rounded-md border px-2 py-1 text-xs font-medium capitalize ${(row?.sInvDocStatus || row?.sInvStatus) === "open" ? "border-success/20 bg-success/10 text-success" : "border-danger/20 bg-danger/10 text-danger"}`}>
    ...
    const openEditModal = async (record: any) => {
            ...
            ? record.sInvBody.map((item: any) => {
                ...
                return normalizeRowKeys({
                    ...
                    customMasters: item?.customMasters &&
                        ...
                        ? { ...item.customMasters }
                        : {},
                    _inventoryBalanceVoucherId: item?._inventoryBalanceVoucherId ||
        ...
        try {
            ...
            inventoryRecords =
                getInventoryBalanceRecords(inventoryResponse);
        }
        catch (error) {
    ...
    useEffect(() => {
        if (!showModal)
            return;
        const headerInventoryFields = (templateFields?.header || []).filter((field: any) => !isTrueValue(field?.isHidden) &&
            Boolean(getInventoryBalanceApiKey(field)));
        if (!headerInventoryFields.length)
            return;
    ...
    const handleAccountSaved = async (savedResponse: any) => {
        try {
            ...
            await dispatch(getAllReportMapping({
            ...
            const createdCustomer = customerAccounts.find((account: any) => (savedCode &&
            ...
            if (createdCustomer) {
                setForm((prev: any) => ({
                    ...prev,
                    sInvCustomerCode: createdCustomer?.accountCode ||
                        prev?.sInvCustomerCode ||
                        "",
                    sInvCustomerName: createdCustomer?.accountName ||
    ...
    const handleProductSaved = async (savedResponse: any) => {
        try {
            ...
            if (transactionsSchema) {
                ...
                updatedData =
                    normalizeSalesInvoiceTemplateFields(loadedData);
            ...
            const createdProduct = selectedOption?.raw ||
                savedProduct ||
                {};
            setForm((previous: any) => {
                ...
                let rowIndex = productTargetRowIndex !== null &&
                    ...
                    ? productTargetRowIndex
                    : updatedProducts.findIndex((row: any) => !row?.productCode &&
                        !row?.productName &&
                        !row?.productId);
                if (rowIndex < 0) {
                    rowIndex =
                ...
                if (selectedField &&
                    selectedOption) {
                    updatedRow =
                        applyMappedFields(selectedField, selectedOption.value, updatedRow);
                }
                const marginProduct = isTrueValue(createdProduct
                    ?.dynamicFields
                    ?.marginProduct ??
                    createdProduct
                        ?.marginProduct);
                updatedRow = {
                    ...updatedRow,
                    sOrderNumber: updatedRow?.sOrderNumber ||
                        previous
                            ?.sInvSalesOrderVoucherNumber ||
                        "",
                    ...
                    productId: createdProduct?._id ||
                        ...
                        "",
                    productDescription: createdProduct
                        ?.productDescription ||
                        updatedRow
                            ?.productDescription ||
                        "",
                ...
                const selectedCustomer = filterAccount?.find((account: any) => account?.accountCode ==
    ...
    const handleRowChange = (index: number, key: string, value: any) => {
        ...
        if (PRODUCT_FIELD_KEYS.has(key) || isInventoryBalanceField(balanceField)) {
            ...
            if (productCode) {
                void loadAvailableQuantity(index, productCode, productType, balanceRow);
            }
        }
        setErrors((prev: any) => ({
            ...prev,
    ...
    const handleSubmit = async () => {
        ...
        const payload: any = {
            ...
            customMasters: form?.customMasters &&
            ...
            sInvBody: products.map((item: any) => {
                ...
                return {
                    ...
                    customMasters: item?.customMasters &&
                        ...
                        : {},
                    dynamicBodyFields: {
                        ...Object.fromEntries(Object.entries(item?.dynamicBodyFields || {}).filter(([fieldKey]) => !CONDITIONAL_MARGIN_FIELD_KEYS.has(fieldKey))),
                        ...(marginProduct
                            ? {
        ...
        try {
            if (editingRecord) {
                ...
                const savedSalesInvoiceVoucherNumber = resolveSavedSalesInvoiceVoucherNumber(result, form?.sInvVoucherNumber);
                if (!savedSalesInvoiceVoucherNumber) {
                    throw new Error("Sales Invoice updated but voucher number was not found, so Inventory Balance update cannot be called");
                }
                ...
                if (!savedSalesInvoiceVoucherNumber) {
                    throw new Error("Sales Invoice created but voucher number was not found, so Inventory Balance save cannot be called");
                }
    ...
    useEffect(() => {
        const prepareFields = async () => {
            if (!transactionsSchema)
                return;
            const hasSchema = Array.isArray(transactionsSchema?.header) ||
    ...
    useEffect(() => {
        dispatch(getAllReportMapping({
            moduleType: "salesInvoice",
        }) as any);
        dispatch(getAllSystemConfigurations({
            offset: 0,
            limit: 100000,
            status: "",
        }) as any);
        const loadAccounts = async () => {
            try {
                ...
                console.log("Failed to load Account Master records", error);
            }
            finally {
                setAccountListLoaded(true);
            }
    ...
    return (<div className="flex h-full w-full flex-col rounded-md border border-border bg-card p-4 text-card-foreground shadow-sm">
        ...
        <DataTable columns={columns} data={salesInvoices} loading={loading} emptyMessage={`No ${status} sales invoice found`} actions={(record: any) => (<div className="flex items-center gap-2">
            ...
            <Permission module="bookez" permissionKey="salesInvoice" action="update">
                <button id="sales-invoice-edit-button" onClick={() => handleEditSalesOrder(record)} className="cursor-pointer rounded-md p-2 text-primary transition-all duration-200 hover:bg-primary/10 hover:text-primary">
                    <Edit size={16} />
                </button>
            </Permission>

            <Permission module="bookez" permissionKey="salesInvoice" action="delete">
                <button id="sales-invoice-delete-button" disabled={deleteLoading} onClick={(e) => handleDeleteSalesInvoiceClick(e, record)} className="cursor-pointer rounded-md p-2 text-danger transition-all duration-200 hover:bg-danger/10 hover:text-danger disabled:opacity-50">
                    <Trash2 size={16} />
                </button>
            </Permission>
        </div>)} />
        ...
        {confirmTooltip.show && (<ConfirmTooltip x={confirmTooltip.x} y={confirmTooltip.y} message="Are you sure you want to delete this sales invoice?" confirmText="Delete" cancelText="Cancel" onConfirm={handleDeleteConfirm} onCancel={() => setConfirmTooltip({
            show: false,
            x: null,
            y: null,
            voucherNumber: null,
        ...
        {!fieldsLoading && (<DynamicAddForm {...{
            ...
            isBodyColumnVisible,
            isBodyCellVisible,
            bodyCellExtraRenderer: (column: any, row: any) => renderSalesInvoiceCellExtra(column, row, enableServiceProductInventory),
            // ★ ADDED: Shared Account Master modal props
            checkAccount,
            setCheckAccount,
            onAccountSaved: handleAccountSaved,
            headerRightContent: posPostingField && !isTrueValue(posPostingField?.isHidden) ? (<div className="flex items-center gap-2">
                <span className={`text-sm font-semibold transition-colors ${posPostingEnabled ? "text-primary" : "text-foreground"}`}>{posPostingField?.label || "POS Posting"}{isTrueValue(posPostingField?.isRequired) && <span className="text-danger">*</span>}</span>
                <button type="button" role="switch" aria-label={posPostingField?.label || "POS Posting"} aria-checked={posPostingEnabled} disabled={posPostingDisabled} onClick={() => {
                    const nextValue = !posPostingEnabled; handleMainChange(posPostingFieldKey, nextValue); if (posPostingFieldKey !== "isAutoPost")
                        handleMainChange("isAutoPost", nextValue);
                }} className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full border transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${posPostingEnabled ? "border-primary bg-primary shadow-md ring-2 ring-primary/30" : "border-border bg-muted-foreground/30 shadow-inner"}`}>
                    <span className={`h-4 w-4 rounded-full bg-white shadow-md transition-transform duration-200 ${posPostingEnabled ? "translate-x-6" : "translate-x-1"}`} />
                </button>
            </div>) : null,
        }} />)}

        <ProductMasterModal show={checkProduct} setShow={(value: boolean) => {
            setCheckProduct(value);
            if (!value) {
                setProductTargetRowIndex(null);
                setProductSearchValue("");
            }
        }} onSaved={handleProductSaved} title="Add New Product" initialProductName={productSearchValue} />

        <Modal show={showPurchaseOrderModal} setShow={setShowPurchaseOrderModal} title="Select Sales Order" state={false} handleSubmit={handlePurchaseOrderConfirm} handleClose={handlePurchaseOrderModalClose} gridCols={1} maxWidth="2xl" modalClassName="rounded-xl" headerClassName="bg-card" footerClassName="bg-card" bodyClassName="!block !p-0 bg-card text-card-foreground" body={<div className="flex h-[520px] flex-col bg-card text-card-foreground">
            <div className="border-b border-border p-5">
                <input value={purchaseOrderSearch} onChange={(e) => setPurchaseOrderSearch(e.target.value)} placeholder="Search Sales Order..." className="w-full rounded-xl border border-border bg-input px-4 py-3 text-sm font-medium text-foreground outline-none transition placeholder:text-muted-foreground focus:border-primary focus:bg-input focus:ring-2 focus:ring-primary/20" />
            </div>

            <div className="min-h-0 flex-1 overflow-y-auto p-5">
                {orderLoader ? (<div className="flex h-full items-center justify-center text-sm font-medium text-muted-foreground">
                    Loading Sales Order...
                </div>) : !salesOrders?.length ? (<div className="flex h-full items-center justify-center text-sm font-medium text-muted-foreground">
                    ...
                    {salesOrders?.map((e: any, index: number) => {
                        ...
                        return (<button key={poNumber || index} type="button" onClick={() => handlePurchaseOrderSelect(e)} className={`w-full rounded-xl border px-4 py-4 text-left transition ${isSelected
                            ...
                            <div className="flex items-center justify-between gap-3">
                                ...
                                {isSelected && (<span className="rounded-full bg-primary px-3 py-1 text-xs font-semibold text-primary-foreground">
                                    Selected
                                </span>)}
                            </div>
                        </button>);
