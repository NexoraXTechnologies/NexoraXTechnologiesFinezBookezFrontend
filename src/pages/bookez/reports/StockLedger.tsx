...
import { createStockLedger, clearStockLedgerData } from "../../../redux/slices/professionalSlice/ledgerReports/stockLedgerSlice";
...
import { formatDateWithCurrentTime, formatProductType, getFirstDateOfCurrentMonth, loadAllTemplateOptions, todayYMD } from "../../../utils/helperFunctions";
...
import { getCustomMasterListing, getCustomMasterModules } from "../../../redux/slices/professionalSlice/customMasterModuleSlice";
import { toast } from "react-toastify";
type StockLedgerProps = { show?: boolean; };
const openingStockViewInputData = {
    header: [
        { key: "openingStockVoucherNumber", label: "Voucher No", type: "text", disabled: true },
        { key: "openingStockDate", label: "Date", type: "date", disabled: true },
        { key: "remark", label: "Remark", type: "textarea", required: false, disabled: true, placeholder: "Remark", colSpan: "full" },
    ],
    body: [
        { key: "productCode", title: "Product", type: "text", width: "240px", disabled: true },
        { key: "description", title: "Description", type: "text", width: "220px", disabled: true },
        { key: "remarks", title: "Remarks", type: "text", width: "180px", disabled: true },
        { key: "quantity", title: "Qty", type: "number", width: "120px", disabled: true, align: "right" },
        { key: "unit", title: "Unit", type: "text", width: "150px", disabled: true },
        { key: "rate", title: "Rate", type: "number", width: "130px", disabled: true, align: "right" },
        { key: "grossAmount", title: "Gross", type: "number", width: "130px", disabled: true, align: "right" },
        { key: "discountPercentage", title: "Disc %", type: "number", width: "110px", disabled: true, align: "right" },
        { key: "discountAmount", title: "Disc Amt", type: "number", width: "130px", disabled: true, align: "right" },
        { key: "cgstPercentage", title: "CGST %", type: "number", width: "110px", disabled: true, align: "right" },
        { key: "cgstAmount", title: "CGST Amt", type: "number", width: "130px", disabled: true, align: "right" },
        { key: "sgstPercentage", title: "SGST %", type: "number", width: "110px", disabled: true, align: "right" },
        { key: "sgstAmount", title: "SGST Amt", type: "number", width: "130px", disabled: true, align: "right" },
        { key: "igstPercentage", title: "IGST %", type: "number", width: "110px", disabled: true, align: "right" },
        { key: "igstAmount", title: "IGST Amt", type: "number", width: "130px", disabled: true, align: "right" },
        { key: "otherAmount", title: "Other", type: "number", width: "130px", disabled: true, align: "right" },
    ...
    footer: [
        { key: "totalQuantity", label: "Total Quantity", value: "0", rawValue: "0" },
        { key: "totalGrossAmount", label: "Gross Amount", value: "0.00", rawValue: "0.00" },
        { key: "totalTaxAmount", label: "Tax Amount", value: "0.00", rawValue: "0.00" },
        { key: "totalNetAmount", label: "Net Amount", value: "0.00", rawValue: "0.00" },
    ]
};
const mainColumns = [
    {
        ...
        render: (row: any) => <span className="font-medium text-card-foreground">{row?.voucherNumber || row?.voucherNo || "-"}</span>
    },
    {
        key: "voucherDate",
        title: "Voucher Date",
        render: (row: any) => {
            ...
            const date = rawDate ? new Date(rawDate).toLocaleDateString("en-IN") : "-";
            return (
    ...
    },
    {
        key: "party",
        title: "Party",
        render: (row: any) => <span className="font-medium text-card-foreground">{row?.party || row?.party || "-"}</span>
    },
    {
        key: "inwardQty",
        title: "Inward Qty",
        render: (row: any) => <span className="font-semibold text-success">{Number(row?.inwardQty || 0).toFixed(2)}</span>
    },
    {
        key: "outwardQty",
        title: "Outward Qty",
        render: (row: any) => <span className="font-semibold text-danger">{Number(row?.outwardQty || 0).toFixed(2)}</span>
    },
    {
        key: "status",
        title: "Status",
        render: (row: any) => {
            const inwardQty = Number(row?.inwardQty || row?.inQty || row?.purchaseQty || row?.receivedQty || 0
            );
            const outwardQty = Number(row?.outwardQty || row?.outQty || row?.salesQty || row?.issuedQty || 0
            );
            if (inwardQty > 0) return <span className="rounded-full bg-success/10 px-3 py-1 text-xs font-bold text-success">Inward</span>;
            if (outwardQty > 0) return <span className="rounded-full bg-danger/10 px-3 py-1 text-xs font-bold text-danger">Outward</span>;
            return <span className="rounded-full bg-muted px-3 py-1 text-xs font-bold text-muted-foreground">-</span>;
        }
    },
];
...
type InventoryFilterOption = { label: string; value: string; raw?: any; };
const getArrayRecords = (response: any): any[] => {
    const roots = [response, response?.data, response?.data?.data, response?.result, response?.payload];
...
const normalizeInventoryMasterName = (value: any) => String(value || "").trim().toLowerCase();
const getInventoryRecordValue = (record: any, keys: string[]) => {
    const nested = record?.data && typeof record.data === "object" ? record.data : record?.dynamicFields && typeof record.dynamicFields === "object"
        ? record.dynamicFields : record?.customFields && typeof record.customFields === "object" ? record.customFields : {};
...
const StockLedger = ({ show = true }: StockLedgerProps) => {
    ...
    const { stockLedgerData = null, listingLoader = false } = useSelector((s: any) => s.stockLedger);
    const { products = [], loading: productLoading = false } = useSelector((s: any) => s.productMaster);
    ...
    const [inventoryFilterOptions, setInventoryFilterOptions] = useState<Record<InventoryFilterKey, InventoryFilterOption[]>>({ warehouse: [], location: [], bin: [], batch: [] });
    ...
    const [viewTemplateFields, setViewTemplateFields] = useState<any>({ header: [], body: [], footer: [] });
    const [viewBodyKey, setViewBodyKey] = useState("products");
    const getVoucherRecordFromResponse = (res: any,
        ...
        const isSameVoucher = (item: any) => {
            ...
            return voucherKeys.some((key) => normalizeVoucherNo(item?.[key]) === normalizedVoucher);
        };
        ...
        const records = Array.isArray(res) ? res : Array.isArray(res?.items) ? res.items : Array.isArray(res?.records) ? res.records : Array.isArray(res?.openingStock) ? res.openingStock
            : Array.isArray(res?.docs) ? res.docs : Array.isArray(res?.data) ? res.data
                : Array.isArray(res?.data?.items) ? res.data.items : Array.isArray(res?.data?.records) ? res.data.records
                    : Array.isArray(res?.data?.openingStock) ? res.data.openingStock : Array.isArray(res?.data?.docs) ? res.data.docs : [];
    ...
    const normalizeSalesReturnForView = (record: any) => {
        ...
        return {
            ...record,
            sInvReturnVoucherNumber: record?.sInvReturnVoucherNumber || record?.voucherNumber || "",
            sInvReturnVoucherDate: record?.sInvReturnVoucherDate || record?.voucherDate || "",
            sInvVoucherNumber: record?.sInvVoucherNumber || "",
            sInvReturnCustomerName: record?.sInvReturnCustomerName || record?.customerName || "",
            sInvReturnCustomerCode: record?.sInvReturnCustomerCode || record?.sInvCustomerCode || "",
    ...
    const normalizePurchaseInvoiceForView = (record: any) => {
        ...
        return {
            ...
            pInvVendorCode: record?.pInvVendorCode || record?.vendorCode || record?.accountCode || "",
            pInvVendorName: record?.pInvVendorName || record?.vendorName || record?.accountName || "",
    ...
    const normalizePurchaseReturnForView = (record: any) => {
        ...
        return {
            ...
            pRetVendorCode: record?.pRetVendorCode || record?.vendorCode || record?.accountCode || "",
            pRetVendorName: record?.pRetVendorName || record?.vendorName || record?.accountName || "",
    ...
    const normalizeGrnForView = (record: any) => {
        ...
        const products = (record?.grnBody || record?.products || record?.body || []
        ).map((item: any) => ({
            ...item,
            productCode: item?.productCode || "",
            productName: item?.productName || "",
            ...
            description: item?.description || item?.productDescription || "",
            productHSNCode: item?.productHSNCode || "",
            quantity: item?.quantity || Number(item?.acceptedQuantity || 0) +
        ...
        return {
            ...record,
            grnVoucherNumber: record?.grnVoucherNumber || record?.voucherNumber || record?.voucherNo || "",
            grnVoucherDate: record?.grnVoucherDate || record?.voucherDate || record?.date || "",
            pOrdVoucherNumber: record?.pOrdVoucherNumber || record?.purchaseOrderVoucherNumber || "",
            grnVendorCode: record?.grnVendorCode || record?.vendorCode || record?.accountCode || "",
            grnVendorName: record?.grnVendorName || record?.vendorName || record?.accountName || record?.party || "",
            grnStatus: record?.grnStatus || record?.status || "open",
            grnRemark: record?.grnRemark || record?.remark || "",
            products,
            grnBody: products,
            grossAmount: footer?.grossAmount || footer?.totalGrossAmount || record?.grossAmount || "0.00",
            discountAmount: footer?.discountAmount || footer?.totalDiscountAmount || record?.discountAmount || "0.00",
            cgstAmount: footer?.cgstAmount || footer?.totalCgstAmount || record?.cgstAmount || "0.00",
            sgstAmount: footer?.sgstAmount || footer?.totalSgstAmount || record?.sgstAmount || "0.00",
            igstAmount: footer?.igstAmount || footer?.totalIgstAmount || record?.igstAmount || "0.00",
            taxAmount: footer?.taxAmount || footer?.totalTaxAmount || record?.taxAmount || "0.00",
            otherAmount: footer?.otherAmount || footer?.totalOtherAmount || record?.otherAmount || "0.00",
            netAmount: footer?.netAmount || footer?.totalNetAmount || record?.netAmount || "0.00",
            adjustedAmount: footer?.adjustedAmount || record?.adjustedAmount || "0.00",
            balanceAmount: footer?.balanceAmount || footer?.netAmount || footer?.totalNetAmount || record?.balanceAmount || "0.00",
            totalQuantity: footer?.totalQuantity || record?.totalQuantity || products.reduce(
    ...
    const normalizeOpeningStockForView = (record: any) => {
        ...
        const products = (record?.openingStockBody || record?.products || record?.body || []
        ).map((item: any) => ({
            ...item,
            productCode: item?.productCode || item?.product || "",
            productName: item?.productName || "",
            productId: item?.productId || "",
            description: item?.description || item?.productDescription || item?.productDescription || "",
        ...
        return {
            ...record,
            openingStockVoucherNumber: record?.openingStockVoucherNumber || record?.voucherNumber || record?.voucherNo || "",
            openingStockDate: record?.openingStockDate || record?.voucherDate || record?.date || "",
            openingStockStatus: record?.openingStockStatus || record?.status || "open",
            openingStockRemark: record?.openingStockRemark || record?.remark || "",
            remark: record?.remark || record?.openingStockRemark || "",
            ...
            grossAmount: footer?.grossAmount || footer?.totalGrossAmount || "0.00",
            discountAmount: footer?.discountAmount || footer?.totalDiscountAmount || "0.00",
            cgstAmount: footer?.cgstAmount || footer?.totalCgstAmount || footer?.totalCGSTAmount || "0.00",
            sgstAmount: footer?.sgstAmount || footer?.totalSgstAmount || footer?.totalSGSTAmount || "0.00",
            igstAmount: footer?.igstAmount || footer?.totalIgstAmount || footer?.totalIGSTAmount || "0.00",
            taxAmount: footer?.taxAmount || footer?.totalTaxAmount || "0.00",
            otherAmount: footer?.otherAmount || footer?.totalOtherAmount || "0.00",
            netAmount: footer?.netAmount || footer?.totalNetAmount || "0.00",
    ...
    const getStockVoucherNumber = (row: any) => {
        return (row?.voucherNumber || row?.voucherNo || row?.sInvVoucherNumber ||
            row?.sInvReturnVoucherNumber || row?.pInvVoucherNumber || row?.pRetVoucherNumber || row?.grnVoucherNumber || row?.openingStockVoucherNumber || ""
        );
    };
    ...
    const handleViewVoucher = async (row: any) => {
        ...
        try {
            ...
            if (config.manualSchema) setViewTemplateFields(config.inputData);
            else await dispatch(getAllTransactionSchema(config.schemaKey) as any);
            const res = await dispatch(config.action(config.params(voucherNumber)) as any).unwrap();
            const record = getVoucherRecordFromResponse(res,
                voucherNumber,
                config.voucherKeys
            );
            if (!record) {
            ...
            const errorMessage = error?.message || error?.payload?.message || config.notFoundMessage || "Voucher not found";
    ...
    const productOptions = useMemo(() => {
        return (products || [])
            .map((item: any) => ({
                label: item?.productName || item?.productDescription || item?.name || "",
    ...
    useEffect(() => {
        dispatch(getAllProducts({ offset: 0, limit: 100, search: "" }) as any);
    }, [dispatch]);
    useEffect(() => {
        const loadInventoryMasters = async () => {
            try {
                setInventoryFiltersLoading(true);
                const moduleResponse = await dispatch(getCustomMasterModules({ offset: 0, limit: 1000, search: "", status: "active" })).unwrap();
                ...
                const loadedEntries = await Promise.all(
                    (Object.keys(foundModules) as InventoryFilterKey[]).map(async (key) => {
                        ...
                        try {
                            const listingResponse = await dispatch(getCustomMasterListing({ moduleCode, offset: 0, limit: 1000, search: "" })).unwrap();
                            return [key, buildInventoryMasterOptions(getArrayRecords(listingResponse), key)] as const;
                        } catch (error) {
                ...
                const nextOptions: any = { warehouse: [], location: [], bin: [], batch: [] };
                loadedEntries.forEach(([key, options]) => { nextOptions[key] = options; });
    ...
    useEffect(() => {
        if (!show) resetStockLedger();
    }, [show]);
    useEffect(() => () => { dispatch(clearStockLedgerData()); }, [dispatch]);
    useEffect(() => {
        if (!productCode) return;
        dispatch(createStockLedger({ productCode, fromDate: formatDateWithCurrentTime(fromDate), toDate: formatDateWithCurrentTime(toDate), warehouseCode, locationCode, batchNumber, binCode }) as any);
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await dispatch(createStockLedger({ productCode, fromDate, toDate, warehouseCode, locationCode, batchNumber, binCode, exportType: "pdf" }) as any).unwrap();
            if (res?.blob) downloadBlobFile(res.blob, `stock-ledger-${productCode}.pdf`);
        } catch (error) {
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await dispatch(createStockLedger({ productCode, fromDate, toDate, warehouseCode, locationCode, batchNumber, binCode, exportType: "excel" }) as any).unwrap();
            if (res?.blob) downloadBlobFile(res.blob, `stock-ledger-${productCode}.xlsx`);
        } catch (error) {
    ...
    const closingStock = Number(stockLedgerData?.balanceQuantity || stockLedgerData?.closingStock || stockLedgerData?.closingQty || 0).toFixed(2);
    const viewFooterTotals = useMemo(() => {
        return {
            ...
            totalGrossAmount: viewForm?.totalGrossAmount || viewForm?.grossAmount || "0.00",
            totalTaxAmount: viewForm?.totalTaxAmount || viewForm?.taxAmount || "0.00",
            totalNetAmount: viewForm?.totalNetAmount || viewForm?.netAmount || "0.00"
        };
    }, [viewForm]);
    ...
    const viewInputData = useMemo(() => ({ ...viewTemplateFields, footer: viewFooterArray }), [viewTemplateFields, viewFooterArray]);
    return (
        <div className="flex h-full w-full flex-col gap-4 bg-background p-4 text-foreground">
            <div className="w-full">
                <button type="button" onClick={() => setFiltersOpen((prev) => !prev)} className="mb-1 cursor-pointer flex w-full items-center justify-between rounded-md border border-border bg-card px-4 py-2.5 text-left transition-colors duration-200 hover:bg-muted/40">
                    <span className="text-sm font-semibold text-card-foreground">Stock Ledger Filters</span>
                    <motion.span animate={{ rotate: filtersOpen ? 180 : 0 }} transition={{ duration: 0.25, ease: "easeInOut" }} className="flex text-muted-foreground"><ChevronDown size={18} /></motion.span>
                </button>
                <AnimatePresence initial={false}>
                    {filtersOpen && (
                        <motion.div key="stock-ledger-filters" initial={{ height: 0, opacity: 0, y: -6 }} animate={{ height: "auto", opacity: 1, y: 0 }} exit={{ height: 0, opacity: 0, y: -6 }} transition={{ height: { duration: 0.3, ease: "easeInOut" }, opacity: { duration: 0.2 }, y: { duration: 0.25, ease: "easeOut" } }} className="overflow-hidden">
                            <div className="w-full [&>*]:rounded-md [&>*]:!p-4 [&_h3]:!hidden [&_h2]:!hidden [&_p]:!text-sm [&_label]:!text-xs [&_input]:!h-10 [&_input]:!text-sm [&_select]:!h-10 [&_select]:!text-sm [&_.text-xl]:!text-lg [&_.text-lg]:!text-base">
                                <ReportFilterCard
                                    title=""
                                    fields={[
                                        { key: "fromDate", type: "date", label: "From Date", value: fromDate, onChange: (value) => { setFromDate(value); }, required: true },
                                        { key: "toDate", type: "date", label: "To Date", value: toDate, onChange: (value) => { setToDate(value); }, required: true },
                                        { key: "productCode", type: "select", label: "Product", placeholder: productLoading ? "Loading products..." : "Select Product", value: productCode, options: productOptions, disabled: productLoading, onChange: (value) => { setProductCode(value); }, required: true },
                                        // @ts-ignore
                                        ...(showWarehouse ? [{ key: "warehouseCode", type: "select", label: "Warehouse", placeholder: inventoryFiltersLoading ? "Loading warehouses..." : "All Warehouses", value: warehouseCode, options: inventoryFilterOptions.warehouse, disabled: inventoryFiltersLoading, onChange: (value: string) => { setWarehouseCode(value); }, required: false }] : []),
                                        // @ts-ignore
                                        ...(showLocation ? [{ key: "locationCode", type: "select", label: "Location", placeholder: inventoryFiltersLoading ? "Loading locations..." : "All Locations", value: locationCode, options: inventoryFilterOptions.location, disabled: inventoryFiltersLoading, onChange: (value: string) => { setLocationCode(value); }, required: false }] : []),
                                        // @ts-ignore
                                        ...(showBin ? [{ key: "binCode", type: "select", label: "Bin", placeholder: inventoryFiltersLoading ? "Loading bins..." : "All Bins", value: binCode, options: inventoryFilterOptions.bin, disabled: inventoryFiltersLoading, onChange: (value: string) => { setBinCode(value); }, required: false }] : []),
                                        // @ts-ignore
                                        ...(showBatch ? [{ key: "batchNumber", type: "select", label: "Batch", placeholder: inventoryFiltersLoading ? "Loading batches..." : "All Batches", value: batchNumber, options: inventoryFilterOptions.batch, disabled: inventoryFiltersLoading, onChange: (value: string) => { setBatchNumber(value); }, required: false }] : []),
                                    ]}
                                    gridCols="4"
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-1 rounded-md border border-border bg-card px-4 py-2 text-sm">
                <div><span className="font-medium text-muted-foreground">Product Type:</span> <span className="font-semibold text-card-foreground">{formatProductType(stockLedgerData?.productType) || "-"}</span></div>
                <div><span className="font-medium text-muted-foreground">UOM:</span> <span className="font-semibold text-card-foreground">{stockLedgerData?.uom || "-"}</span></div>
                <div><span className="font-medium text-muted-foreground">Available Quantity:</span> <span className="font-semibold text-card-foreground">{closingStock}</span></div>
            </div>
            <DataTable
                columns={mainColumns}
                data={tableData}
                ...
                actions={(row: any) => (
                    <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); handleViewVoucher(row); }}
                        className="inline-flex items-center gap-1 rounded-lg bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary transition hover:bg-primary/20 cursor-pointer"
                    >
                        <Eye size={15} />
                    </button>
                )}
            ...
            <DynamicAddForm
                ...
                loading={viewLoading}
                onClose={() => { setViewModal(false); setViewForm({}); setViewErrors({}); }}
                onSubmit={() => { }}
                form={viewForm}
                errors={viewErrors}
                handleAddRow={() => { }}
