...
const mergeTripLRCollectionForm = (data: any = {}) => {
    ...
    return {
        ...
        lrTouchUp: Array.isArray(data.lrTouchUp)
            ? data.lrTouchUp.map((item: any) => ({
                transportTouchupNumber: item?.transportTouchupNumber || "",
                touchUpId: item?.touchUpId || "",
                pickupDetails: item?.pickupDetails && typeof item.pickupDetails === "object" ? { ...item.pickupDetails } : {},
                deliveryDetails: item?.deliveryDetails && typeof item.deliveryDetails === "object" ? { ...item.deliveryDetails } : {},
                material: item?.material || "",
...
const toTripLRCollectionPayload = (form: any, overrides: any = {}) => {
    ...
    return {
        ...
        lrTouchUp: (merged.lrTouchUp || []).map((item: any) => {
            const transportTouchupNumber = String(item?.transportTouchupNumber || "").trim();
            ...
            const newImageBase64 = typeof touchUpLr === "string" && touchUpLr.startsWith("data:image/")
                ...
                : typeof touchUpLr?.base64 === "string" && touchUpLr.base64.startsWith("data:image/")
                    ...
                    : "";
            const existingTouchUpLr = typeof touchUpLr === "string"
                ? touchUpLr.trim() && !touchUpLr.startsWith("data:image/") ? touchUpLr : {}
                : touchUpLr && typeof touchUpLr === "object" && Object.keys(touchUpLr).length
                    ? { ...touchUpLr }
                    : {};
...
const CreateEditTripLREntry = () => {
    ...
    const mode = location.state?.mode || pathnameMode;
    ...
    const pageTitle = isView ? "View Trip L/R Collection" : isEdit ? "Edit Trip L/R Collection" : "Create Trip L/R Collection";
    const pageDescription = isView
        ? "View LR collection details."
        : isEdit
            ? "Update LR collection details."
            : "Create LR collection with trip, route, vehicle, driver, cargo and freight details.";
    ...
    const allTransportTouchUpOptions = useMemo(() => {
        ...
        return (transportTouchups || [])
            ...
            .flatMap((item: any) => {
                const transportTouchupNumber = item?.transportTouchupNumber || "";
                return (Array.isArray(item?.touchUp) ? item.touchUp : []).map((touchUp: any, index: number) => {
                    const touchUpId = touchUp?.touchUpId || `TouchUp-${String(index + 1).padStart(3, "0")}`;
                    const uniqueKey = `${transportTouchupNumber}__${touchUpId}`;
                    return { label: `${transportTouchupNumber} - ${touchUpId}`, value: uniqueKey, transportTouchupNumber, touchUpId, raw: touchUp };
                });
            });
    }, [transportTouchups, form.transportOrderNumber, form.tripNumber]);

    const transportTouchUpOptions = useMemo(() => {
        const selectedIds = new Set((form.lrTouchUp || []).map((item: any) => `${item?.transportTouchupNumber || ""}__${item?.touchUpId || ""}`));
        return allTransportTouchUpOptions.filter((option: any) => !selectedIds.has(String(option.value)));
    }, [allTransportTouchUpOptions, form.lrTouchUp]);

    const getTouchUpOption = (uniqueKey: any) => allTransportTouchUpOptions.find((option: any) => String(option?.value || "") === String(uniqueKey || ""));

    const normalizeTouchUpMatchValue = (value: any) => String(value || "").trim().toLowerCase();

    const getTouchUpMatchScore = (item: any, option: any) => {
        const raw = option?.raw || {};
        const itemPickup = item?.pickupDetails || {};
        const itemDelivery = item?.deliveryDetails || {};
        const optionPickup = raw?.pickupLocation || {};
        const optionDelivery = raw?.deliveryLocation || {};
        let score = 0;

        const compare = (left: any, right: any, points = 1) => {
            const a = normalizeTouchUpMatchValue(left);
            const b = normalizeTouchUpMatchValue(right);
            if (a && b && a === b) score += points;
        };

        compare(itemPickup?.name, optionPickup?.name, 4);
        compare(itemPickup?.address, optionPickup?.address, 4);
        compare(itemPickup?.city, optionPickup?.city, 2);
        compare(itemPickup?.state, optionPickup?.state, 1);
        compare(itemDelivery?.name, optionDelivery?.name, 4);
        compare(itemDelivery?.address, optionDelivery?.address, 4);
        compare(itemDelivery?.city, optionDelivery?.city, 2);
        compare(itemDelivery?.state, optionDelivery?.state, 1);
        compare(item?.material, raw?.material, 3);
    ...
    const handleTransportTouchUpSelect = (uniqueKey: string) => {
        if (!uniqueKey) return;
        const selected = getTouchUpOption(uniqueKey);
        if (!selected?.raw) return;

        setForm((prev: any) => {
            const existing = Array.isArray(prev.lrTouchUp) ? prev.lrTouchUp : [];
            const alreadyExists = existing.some((item: any) => String(item?.transportTouchupNumber || "") === String(selected.transportTouchupNumber || "") && String(item?.touchUpId || "") === String(selected.touchUpId || ""));
            if (alreadyExists) return { ...prev, transportTouchUp: "" };
            return {
                ...prev,
                transportTouchUp: "",
                lrTouchUp: [
                    ...existing,
                    {
                        transportTouchupNumber: selected.transportTouchupNumber,
                        touchUpId: selected.touchUpId,
                        pickupDetails: mapTouchUpLocation(selected.raw?.pickupLocation),
                        deliveryDetails: mapTouchUpLocation(selected.raw?.deliveryLocation),
                        material: selected.raw?.material || "",
                        touchUpLr: {},
    ...
    const handleRemoveLRTouchUp = (transportTouchupNumber: string, touchUpId: string) => {
        if (isView) return;

        setForm((prev: any) => ({
            ...prev,
            lrTouchUp: (prev.lrTouchUp || []).filter((item: any) => !(String(item?.transportTouchupNumber || "") === String(transportTouchupNumber || "") && String(item?.touchUpId || "") === String(touchUpId || ""))),
        }));
    };

    const handleTouchUpImageChange = async (transportTouchupNumber: string, touchUpId: string, fallbackIndex: number, file: File | undefined) => {
        if (isView || !file) return;
        if (!String(file.type || "").startsWith("image/")) {
            toast.warn("Please select an image file");
            return;
        }
        ...
        try {
            ...
            setForm((prev: any) => {
                const lrTouchUp = [...(prev.lrTouchUp || [])];
                const exactIndex = lrTouchUp.findIndex((item: any) => String(item?.transportTouchupNumber || "") === String(transportTouchupNumber || "") && String(item?.touchUpId || "") === String(touchUpId || ""));
                const targetIndex = exactIndex >= 0 ? exactIndex : fallbackIndex;
                const current = lrTouchUp[targetIndex];
                if (!current) return prev;
    ...
    const resolveLRTouchUps = (items: any[] = []) => {
        const sourceItems = Array.isArray(items) ? items : [];
        if (!sourceItems.length || !allTransportTouchUpOptions.length) return sourceItems;

        const reservedKeys = new Set<string>();
        const exactByIndex = new Map<number, any>();

        sourceItems.forEach((item: any, index: number) => {
            const transportTouchupNumber = String(item?.transportTouchupNumber || "").trim();
            const touchUpId = String(item?.touchUpId || "").trim();
            if (!transportTouchupNumber || !touchUpId) return;

            const option = allTransportTouchUpOptions.find((entry: any) => String(entry?.transportTouchupNumber || "") === transportTouchupNumber && String(entry?.touchUpId || "") === touchUpId);
            if (!option?.raw) return;

            exactByIndex.set(index, option);
            reservedKeys.add(String(option?.value || ""));
        });

        const usedKeys = new Set<string>();

        return sourceItems.map((item: any, index: number) => {
            let option = exactByIndex.get(index);
            ...
            if (!option) {
                ...
                if (candidates.length === 1) option = candidates[0];
                    ...
                    const ranked = candidates.map((entry: any) => ({ entry, score: getTouchUpMatchScore(item, entry) })).sort((a: any, b: any) => b.score - a.score);
                    option = ranked[0]?.entry || candidates[0];
                }
            }

            if (!option?.raw) return item;

            usedKeys.add(String(option?.value || ""));

            return {
                ...item,
                transportTouchupNumber: String(option?.transportTouchupNumber || item?.transportTouchupNumber || "").trim(),
                touchUpId: String(option?.touchUpId || item?.touchUpId || "").trim(),
                pickupDetails: mapTouchUpLocation(option.raw?.pickupLocation),
                deliveryDetails: mapTouchUpLocation(option.raw?.deliveryLocation),
                material: option.raw?.material || item?.material || "",
                touchUpLr: item?.touchUpLr,
            };
        });
    };
    ...
    useEffect(() => {
        ...
        setForm((prev: any) => {
            const currentTouchUps = Array.isArray(prev.lrTouchUp) ? prev.lrTouchUp : [];
            const nextTouchUps = resolveLRTouchUps(currentTouchUps);
            return JSON.stringify(currentTouchUps) === JSON.stringify(nextTouchUps) ? prev : { ...prev, lrTouchUp: nextTouchUps };
        });
    }, [allTransportTouchUpOptions]);
    ...
    const renderFields = (fields: any[]) =>
        fields.map((field: any) => (
            <Fragment key={field.key}>
                {renderField({
                    field: isView ? { ...field, disabled: true } : field,
                    form: fieldForm,
                    handleInputChange,
                    handleSelectChange,
                    updateField,
                        ...
                        <div className="w-full md:col-span-2 xl:col-span-4">
                            ...
                            {(form.lrTouchUp || []).length > 0 && (
                                <div className="mt-4 overflow-hidden rounded-xl border border-border bg-muted/20">
                                    ...
                                    <div className="grid grid-cols-1 gap-3 p-3 xl:grid-cols-2">
                                        {(form.lrTouchUp || []).map((item: any, index: number) => {
                                            const option = getTouchUpOption(`${item?.transportTouchupNumber || ""}__${item?.touchUpId || ""}`);
                                            const touchUp = option?.raw || {};
                                            const pickupDetails = option?.raw ? mapTouchUpLocation(touchUp?.pickupLocation) : mapTouchUpLocation(item?.pickupDetails);
                                            const deliveryDetails = option?.raw ? mapTouchUpLocation(touchUp?.deliveryLocation) : mapTouchUpLocation(item?.deliveryDetails);
                                            const pickup = pickupDetails?.name || pickupDetails?.city || "-";
                                            const delivery = deliveryDetails?.name || deliveryDetails?.city || "-";
                                            const material = option?.raw ? touchUp?.material || item?.material || "-" : item?.material || "-";
                                            const imageSource = getTouchUpImageSource(item?.touchUpLr);
                                            const hasNewPhoto = Boolean(
                                                (typeof item?.touchUpLr === "string" && item.touchUpLr.startsWith("data:image/")) ||
                                                item?.touchUpLr?.base64
                                                ...
                                                <div key={item?.transportTouchupNumber ? `${item.transportTouchupNumber}__${item?.touchUpId || index}` : `${item?.touchUpId || "touchup"}__${index}`} className="group relative flex min-w-0 gap-3 rounded-xl border border-border bg-card p-3 shadow-sm transition hover:border-primary/30 hover:shadow-md">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <div className="flex items-start justify-between gap-2">
                                                            <div className="min-w-0">
                                                                <div className="flex flex-wrap items-center gap-1.5">
                                                                    <p className="truncate text-sm font-bold text-card-foreground">{item?.transportTouchupNumber ? `${item.transportTouchupNumber} - ${item?.touchUpId || `Touch Up ${index + 1}`}` : item?.touchUpId || `Touch Up ${index + 1}`}</p>
                                                                    <span className="max-w-[180px] truncate rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary">{material}</span>
                                                                </div>
                                                                <div className="mt-1.5 flex min-w-0 items-center gap-1.5 text-xs font-medium text-muted-foreground">
                                                                    <span className="truncate">{pickup}</span>
                                                            ...
                                                            {!isView && (
                                                                <button type="button" onClick={() => handleRemoveLRTouchUp(item?.transportTouchupNumber || "", item?.touchUpId || "")} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-danger/10 text-danger transition hover:bg-danger/20" title="Remove Touch Up">
                                                                    <X size={13} />
                                                                </button>
                                                            )}
                                                        </div>
                                                            ...
                                                            {!isView && (
                                                                <label className="inline-flex h-7 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-primary/25 bg-primary/5 px-2.5 text-[11px] font-bold text-primary transition hover:border-primary/40 hover:bg-primary/10">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        className="hidden"
                                                                        onChange={(e) => {
                                                                            handleTouchUpImageChange(item?.transportTouchupNumber || "", item?.touchUpId || "", index, e.target.files?.[0]);
                                                                            e.target.value = "";
                                                                        }}
                                                                    />
                                                                </label>
