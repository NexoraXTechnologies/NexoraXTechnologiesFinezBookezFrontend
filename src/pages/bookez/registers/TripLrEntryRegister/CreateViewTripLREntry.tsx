...
import { buildEWayBillPayload } from "../../transportation/eWayBill/ewaybillservice";
...
const CreateViewTripLREntry = ({
    ...
    const voucherNumber =
        ...
        locationState?.lrNumber ||
        params?.voucherNumber ||
        params?.lrNumber ||
        "";

    const initialLRData = propLRData || locationState?.lrData;

    const [form, setForm] = useState<any>(() =>
        mergeTripLRCollectionForm(initialLRData || createInitialTripLRCollection())
    );
    ...
    const screenDisabled = isView || loading;

    const pageTitle = isView
        ? "View Trip L/R Collection"
        : isEdit
            ? "Edit Trip L/R Collection"
            : "Create Trip L/R Collection";
    ...
    const transportOrderOptions = useMemo(() => {
        ...
        if (isView) {
            return currentVoucher
                ? [{ label: currentVoucher, value: currentVoucher }]
                : [];
        }
        ...
        return transportOrders
            .filter((order: any) => {
                ...
                const normalizedCurrentVoucher = normalizeVoucher(currentVoucher);

                if (isEdit && voucher === normalizedCurrentVoucher) return true;
    ...
    const loadEntry = useCallback(async () => {
        // View mode receives lrData from the register modal.
        // Do not call the LR detail API again.
        if (isView) return;
        if (!isEdit) return;

        const passedData = propLRData || locationState?.lrData;
    ...
    useEffect(() => {
        if (!propLRData) return;
        setForm(mergeTripLRCollectionForm(propLRData));
    }, [propLRData]);

    useEffect(() => {
        // View mode is display-only.
        // The register already fetched the full LR and passes it through lrData,
        // so do not call Transport Order, LR List, or LR Detail APIs again.
        if (isView) return;
    ...
    }, [fetchTransportOrders, fetchLREntries, isView, loadEntry]);
    ...
    const basicFields = [
        {
            ...
            onSearchChange: isView ? undefined : handleTripSearchChange,
        ...
        fields.map((field: any) => (
            <Fragment key={field.key}>
                {renderField({
                    field: {
                        ...field,
                        disabled: screenDisabled || field.disabled,
                    },
                    form: fieldForm,
                    handleInputChange,
                    handleSelectChange,
...
export default CreateViewTripLREntry;