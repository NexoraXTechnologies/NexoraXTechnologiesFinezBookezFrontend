...
const mapCustomMasterOptions = (items: any[]): DropdownOption[] => {
    return (items || [])
        .map((item: any) => ({
            label: String(item?.name || item?.label || item?.code || "").trim(),
            value: String(item?.code || item?.value || item?._id || "").trim(),
        }))
...
const SalesRegister = () => {
    ...
    const [customFilterOptions, setCustomFilterOptions] = useState<Record<string, DropdownOption[]>>({});
    ...
    const getPayload = (
        ...
        return {
            ...
            productCode: product,
            customCodes: selectedCustomCodes.length
                ? selectedCustomCodes
                : [""],
            ...(isExport
                ? {
    ...
    useEffect(() => {
        dispatch(getRegisterFilterDropdowns("salesInvoice"));
            ...
            const optionEntries = await Promise.all(
                customFilters.map(
                    async (filter: CustomFilterDefinition) => {
                        ...
                        try {
                            const customResponse = await professionalAxios.get(apiPath);
                            const items = customResponse?.data?.data?.items || customResponse?.data?.items || customResponse?.data?.data?.data?.items || customResponse?.data?.records || [];
                            return [
                                filter.key,
                                mapCustomMasterOptions(Array.isArray(items) ? items : []),
                            ] as const;
                        } catch (error) {
                            console.log("Custom register filter options failed:", filter?.key, error);
                            return [filter.key, []] as const;
                        }
                    }
                )
    ...
    const openExportPicker = async (type: "pdf" | "excel") => {
        ...
        try {
            ...
            console.log("Sales register export columns failed", error);
            setExportType(null);
        } finally {
            ...
            <RegisterFilterCard
                ...
                fields={[
                    ...
                    ...customFilters.map((filter: any) => ({
                        ...
                        options:
                            customFilterOptions[filter.key] || [],
